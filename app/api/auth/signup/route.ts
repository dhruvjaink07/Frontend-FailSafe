...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResp = await fetch(`${BACKEND_BASE_URL}/internal/auth/signup`, {
      method: "POST",
      cache: "no-store",
    ...
    if (!backendResp.ok) {
      return NextResponse.json({ error: payload.error || payload.message || "Sign-up failed" }, { status: backendResp.status || 503 })
    }
